...
// libs/parmind-skill/src/lib/telemetry.ts
var withTelemetry, withRetry, createSafeLogger;
var init_telemetry = __esm({
  "libs/parmind-skill/src/lib/telemetry.ts"() {
    "use strict";
    withTelemetry = async (logger, method, meta, fn) => {
      const start = Date.now();
      logger.debug(`parmind-skill:${method}:start`, { ...meta, start });
      try {
        const result = await fn();
        logger.info(`parmind-skill:${method}:success`, {
          ...meta,
          durationMs: Date.now() - start
        });
        return result;
      } catch (error) {
        logger.error(`parmind-skill:${method}:error`, {
          ...meta,
          durationMs: Date.now() - start,
    ...
    withRetry = async (fn, attempts = 3, delayMs = 250) => {
      let lastError;
      for (let attempt = 1; attempt <= attempts; attempt++) {
        try {
          ...
          if (attempt === attempts || ![429, 503].includes(status)) {
            break;
          }
...
var T, _, Re, m, Te, Oe, we, I, ye, N, re, se, Pe, Q, Se, F, $e, _e, q, j, Le, ie, Me, U, te, ze, Ce, Ae, Ie, oe, Ee, v, K, ae, Be, le, De, qe, ue, ve, Ge, He, pe, Ze, Ne, ce, Qe, Fe, je, Ue, Ke, We, Xe, Je, Ve, Ye, D, et, he, ke, tt, ne, W, nt, Z, rt, E, z, st, de, w, x, y, $, b, P, B, L, Ut, Kt, Wt, Xt, Jt, Yt, en;
...
var require_matcher = __commonJS({
  "node_modules/node-html-parser/dist/matcher.js"(exports) {
    ...
    function findAll(test, nodes) {
      let result = [];
      for (let i = 0, j2 = nodes.length; i < j2; i++) {
        if (!isTag(nodes[i]))
          continue;
        if (test(nodes[i]))
          result.push(nodes[i]);
...
var require_text = __commonJS({
  "node_modules/node-html-parser/dist/nodes/text.js"(exports) {
    ...
    var node_1 = __importDefault(require_node());
    var type_1 = __importDefault(require_type());
    var TextNode2 = class _TextNode extends node_1.default {
      clone() {
  ...
  "node_modules/node-html-parser/dist/nodes/html.js"(exports) {
    ...
    var HTMLElement2 = class _HTMLElement extends node_1.default {
      ...
      get rawAttributes() {
        ...
        const attrs = {};
        if (this.rawAttrs) {
          const re2 = /([a-zA-Z()[\]#@$.?:][a-zA-Z0-9-._:()[\]#]*)(?:\s*=\s*((?:'[^']*')|(?:"[^"]*")|\S+))?/g;
          let match;
    ...
    function base_parse(data, options = {}) {
      ...
      const kBlockTextElements = element_names.map((it2) => new RegExp(`^${it2}$`, "i"));
      const kIgnoreElements = element_names.filter((it2) => Boolean(elements[it2])).map((it2) => new RegExp(`^${it2}$`, "i"));
      function element_should_be_ignore(tag) {
        return kIgnoreElements.some((it2) => it2.test(tag));
      }
      function is_block_text_element(tag) {
        return kBlockTextElements.some((it2) => it2.test(tag));
      }
...
// libs/parmind-skill/src/lib/markdown.ts
var import_node_html_parser, createEmptyNode, ELEMENT_TAGS, TEXT_TAGS, markdownToSlate, transformNode, isEmptyBlock, normalizeChildren, combineTextChildren;
var init_markdown = __esm({
  "libs/parmind-skill/src/lib/markdown.ts"() {
    ...
    markdownToSlate = (markdownText, options = {}) => {
      ...
      try {
        const html = g.parse(markdownText);
        const root = (0, import_node_html_parser.parse)(html, {
          blockTextElements: { pre: true }
        });
        const nodes = root.childNodes.map((node) => transformNode(node, options)).filter(Boolean).filter((node) => !isEmptyBlock(node));
        return nodes.length ? nodes : [createEmptyNode()];
      } catch (error) {
        console.error("markdownToSlate error", error);
        return [
          {
            type: "paragraph",
    ...
    transformNode = (node, options, marks = []) => {
      if (node instanceof import_node_html_parser.TextNode) {
        ...
        if (!text) return null;
        const markProps = Object.assign({}, ...marks);
        return {
          text,
          ...markProps
        };
      }
      if (node instanceof import_node_html_parser.HTMLElement) {
        const tag = node.tagName.toLowerCase();
        if (TEXT_TAGS[tag]) {
          const children = node.childNodes.map((child) => transformNode(child, options, [...marks, TEXT_TAGS[tag]])).filter(Boolean);
          return combineTextChildren(children);
        }
        if (ELEMENT_TAGS[tag]) {
          const slateElement = {
            ...ELEMENT_TAGS[tag](node),
            children: normalizeChildren(
              node.childNodes.map((child) => transformNode(child, options, marks)).filter(Boolean)
            )
          };
          return slateElement;
        }
        return {
          type: options.defaultType ?? "paragraph",
          children: normalizeChildren(
            node.childNodes.map((child) => transformNode(child, options, marks)).filter(Boolean)
          )
        };
    ...
    isEmptyBlock = (node) => {
      ...
      return !!children && children.length === 1 && "text" in children[0] && !children[0].text;
    };
    ...
    combineTextChildren = (children) => {
      ...
      const markProps = Object.assign(
        {},
        ...children.filter((child) => "text" in child).map((child) => {
...
var init_api_key_client = __esm({
  "libs/parmind-skill/src/lib/api-key-client.ts"() {
    ...
    init_telemetry();
    init_markdown();
    PROD_API_BASE = "https://f1.gloow.io";
    ENV_API_BASE = "PARMIND_API_BASE";
    ENV_API_KEY = "PARMIND_API_KEY";
    createApiKeyClient = (options = {}) => {
      const apiKey = options.apiKey ?? process.env[ENV_API_KEY];
      if (!apiKey) {
        throw new Error(
          "Missing Parmind API key. Provide apiKey option or set PARMIND_API_KEY env var."
        );
      }
      const baseUrl = resolveApiBaseUrl(options.apiBaseUrl);
      const logger = createSafeLogger(options.logger);
      return createApiKeyBackedClient({ apiKey, baseUrl, logger });
    };
    ...
    toSlateContent = (input) => {
      ...
      return markdownToSlate(input.markdownContent);
    };
    createApiKeyBackedClient = ({
      apiKey,
      baseUrl,
      logger,
      ensureSlateContent = toSlateContent
    }) => {
      const request = async (path2, init = {}) => {
        const headers = new Headers(init.headers ?? {});
        headers.set("Authorization", `ParmindKey ${apiKey}`);
        if (init.body && !headers.has("content-type")) {
          headers.set("Content-Type", "application/json");
        }
        const url = `${baseUrl}/${path2.replace(/^\/+/, "")}`;
        ...
        if (!response.ok) {
          const message = typeof parsed === "string" ? parsed : parsed?.["message"] ?? parsed?.["error"] ?? response.statusText;
          throw new Error(`Parmind API error ${response.status}: ${message}`);
        }
        return parsed ?? {};
      };
      const unwrap = (payload, context) => {
      ...
      return {
        async createNote(input) {
          return withTelemetry(
            ...
            () => withRetry(async () => {
              ...
              const node = unwrap(response, "node:create");
              if (input.areas?.length) {
                await Promise.all(
                  input.areas.map(
                    (areaId) => request(`areas/${areaId}/nodes/${node.id}`, { method: "POST" })
                  )
                );
              }
        ...
        async listAreas() {
          return withTelemetry(
            ...
            () => withRetry(async () => {
              ...
              return { areas: unwrap(response, "areas:list") };
            })
          );
        },
        async createArea(input) {
          return withTelemetry(
            ...
            () => withRetry(async () => {
              ...
              const area = unwrap(response, "areas:create");
              return { area };
            })
          );
        },
        async applyArea(input) {
          return withTelemetry(
            logger,
            "applyArea",
            { areaId: input.areaId, nodeId: input.nodeId },
            () => withRetry(async () => {
              await request(`areas/${input.areaId}/nodes/${input.nodeId}`, {
                method: "POST",
                body: input.data ? JSON.stringify(input.data) : void 0
              });
            })
          );
        },
          ...
          return withTelemetry(
            ...
            () => withRetry(async () => {
              ...
              const response = await request(
                `search-pg?${params.toString()}`
              );
              const data = unwrap(response, "search-pg");
              return {
                query: input.query,
        ...
        async getNodeContext(nodeId) {
          return withTelemetry(
            ...
            { nodeId },
            () => withRetry(async () => {
              const [nodeResponse, connectionsResponse] = await Promise.all([
                request(`node/${nodeId}`),
                request(
                  `node/${nodeId}/connections`
                )
              ]);
              const node = unwrap(nodeResponse, "node:get");
...
// libs/parmind-skill/src/cli/main.ts
...
var require_main = __commonJS({
  "libs/parmind-skill/src/cli/main.ts"() {
    init_esm();
    init_api_key_client();
    var ENV_KB_ID = "PARMIND_KB_ID";
    var program2 = new Command();
    program2.name("parmind-skill").description("CLI for Parmind knowledge base operations").option("--api-key <key>", "Parmind API key (defaults to PARMIND_API_KEY env)").option("--api-base <url>", "Override Parmind API base URL (defaults to PARMIND_API_BASE env)").option("--kb <kbId>", "Default knowledge base ID (defaults to PARMIND_KB_ID env)").option("--pretty", "Pretty print JSON output", false);
    var getClient = (opts) => createApiKeyClient({
      apiKey: opts.apiKey,
      apiBaseUrl: opts.apiBase
    });
    var resolveKbId = (opts, cmdKb) => {
      const kbId = cmdKb ?? opts.kb ?? process.env[ENV_KB_ID];
      if (!kbId) {
        throw new Error(
          "Knowledge base ID required. Pass --kb <id> or set PARMIND_KB_ID env var."
        );
      }
      return kbId;
    };
    var output = (data, pretty) => {
      const json = pretty ? JSON.stringify(data, null, 2) : JSON.stringify(data);
      process.stdout.write(`${json}
`);
    };
    program2.command("note:create").description("Create a note inside a KB").option("--kb <kbId>", "Knowledge base ID (overrides --kb root option and PARMIND_KB_ID)").requiredOption("--title <title>", "Note title").option("--markdown <text>", "Markdown content string").option("--markdown-file <file>", "Path to a markdown file").option("--area <areaId...>", "One or more area IDs to apply").action(async (cmdOptions) => {
      const rootOpts = program2.opts();
      try {
        const client = getClient(rootOpts);
        const kbId = resolveKbId(rootOpts, cmdOptions.kb);
    ...
    program2.command("note:get").description("Fetch a single note by node ID").requiredOption("--id <nodeId>", "Node ID").action(async (cmdOptions) => {
      const rootOpts = program2.opts();
      try {
        const client = getClient(rootOpts);
        const result = await client.getNote(cmdOptions.id);
        output(result, rootOpts.pretty);
      } catch (error) {
        console.error(error instanceof Error ? error.message : error);
        process.exit(1);
    ...
    program2.command("search").description("Search the KB (full-text)").requiredOption("--query <query>", "Search query").option("--page <page>", "Page number", (v2) => Number(v2), 1).option("--page-size <size>", "Page size", (v2) => Number(v2), 15).action(async (cmdOptions) => {
      const rootOpts = program2.opts();
      try {
        const client = getClient(rootOpts);
    ...
    program2.command("area:list").description("List areas in the KB").action(async () => {
      const rootOpts = program2.opts();
      try {
        const client = getClient(rootOpts);
    ...
    program2.command("area:create").description("Create a new area in the KB").option("--kb <kbId>", "Knowledge base ID (overrides --kb root option and PARMIND_KB_ID)").requiredOption("--name <name>", "Area name").option("--description <text>", "Area description").option("--color <value>", "Optional color token / hex").option("--icon <value>", "Optional icon name").option("--data <json>", "JSON blob for area.data").option("--data-file <file>", "Path to JSON file for area.data").action(async (cmdOptions) => {
      const rootOpts = program2.opts();
      try {
        const client = getClient(rootOpts);
        const kbId = resolveKbId(rootOpts, cmdOptions.kb);
        ...
        if (cmdOptions.data) {
          try {
            data = JSON.parse(cmdOptions.data);
          } catch (err) {
            throw new Error(`Invalid JSON provided to --data: ${err.message}`);
          }
        } else if (cmdOptions.dataFile) {
          const filePath = path.resolve(process.cwd(), cmdOptions.dataFile);
          try {
            data = JSON.parse(fs.readFileSync(filePath, "utf-8"));
          } catch (err) {
            throw new Error(`Failed to read JSON from --data-file: ${err.message}`);
          }
        }
        const result = await client.createArea({
          ...
          color: cmdOptions.color,
          icon: cmdOptions.icon,
          data
        });
        output(result, rootOpts.pretty);
      } catch (error) {
    ...
    program2.command("area:apply").description("Apply an area to an existing node").requiredOption("--area <areaId>", "Area ID").requiredOption("--node <nodeId>", "Node ID").action(async (cmdOptions) => {
      const rootOpts = program2.opts();
      try {
        const client = getClient(rootOpts);
        await client.applyArea({ areaId: cmdOptions.area, nodeId: cmdOptions.node });
        output({ success: true }, rootOpts.pretty);
      } catch (error) {
        console.error(error instanceof Error ? error.message : error);
        process.exit(1);
      }
